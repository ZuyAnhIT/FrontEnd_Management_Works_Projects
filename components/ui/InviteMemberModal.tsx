...
import { X, Mail, Crown, UserPlus, FolderKanban, Briefcase, Search, Loader2 } from "lucide-react";
...
export default function InviteMemberModal({
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Chỉ tìm kiếm nếu có companyId, không phải đang mời vào Company (vì mời vào company là mời người ngoài), và nhập > 1 ký tự
    // Nếu contextType === 'company', ta vẫn có thể gợi ý để check xem user đã tồn tại chưa, tuỳ logic.
  ...
  const renderHeaderIcon = () => {
    switch (contextType) {
      ...
      case "workspace": return <Briefcase className="w-5 h-5 text-purple-600" />;
      default: return <UserPlus className="w-5 h-5 text-blue-600" />;
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="bg-slate-50/50 px-6 py-4 flex justify-end gap-3 border-t border-slate-100 shrink-0">
          <Button onClick={onClose} variant="outline" className="bg-white border-slate-300 text-slate-700 h-9 px-4">
            Cancel
          </Button>
          <Button onClick={onInvite} disabled={isLoading} className="bg-blue-600 hover:bg-blue-700 text-white h-9 px-4">
            {isLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin"/>Sending...</> : "Send Invitation"}
          </Button>
        </div>
