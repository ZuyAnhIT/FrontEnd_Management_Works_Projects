...
export default function NotificationItem({ item, onClick }: NotificationItemProps) {
  ...
  const getActionConfig = (action: string) => {
    switch (action) {
      case "CREATE": 
        return { icon: PlusCircle, color: "text-emerald-600", bg: "bg-emerald-50", label: "created" };
      case "START":
        return { icon: PlayCircle, color: "text-blue-600", bg: "bg-blue-50", label: "started" };
      case "UPDATE": 
        return { icon: FileEdit, color: "text-amber-600", bg: "bg-amber-50", label: "updated" };
      case "MOVE_STATUS":
        return { icon: ArrowRightCircle, color: "text-indigo-600", bg: "bg-indigo-50", label: "moved" };
      case "COMPLETE":
      ...
      case "COMMENT":
        return { icon: MessageSquare, color: "text-sky-600", bg: "bg-sky-50", label: "commented on" };
      default:
        return { icon: GitCommit, color: "text-slate-500", bg: "bg-slate-50", label: "acted on" };
    }
  };
  ...
  const getEntityIcon = (type: string) => {
      switch(type) {
          case "TASK": return <Layout className="w-3.5 h-3.5" />;
          case "SPRINT": return <Layers className="w-3.5 h-3.5" />;
          case "PROJECT": return <FolderKanban className="w-3.5 h-3.5" />;
          case "WORKSPACE": return <Briefcase className="w-3.5 h-3.5" />;
          default: return <GitCommit className="w-3.5 h-3.5" />;
      }
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative shrink-0 mt-1">
        ...
        <div className={`absolute -bottom-1 -right-1 rounded-full p-0.5 shadow-sm border border-white ${config.bg}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-sm font-semibold text-blue-700 mb-1.5 group-hover:text-blue-600 transition-colors">
            {/* Icon loại đối tượng */}
            <span className="opacity-70 shrink-0">{getEntityIcon(item.entityType)}</span>
