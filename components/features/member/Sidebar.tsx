...
export default function MemberSidebar({
  ...
  const [showProjects, setShowProjects] = useState(true); // Mặc định mở projects
  ...
  // Menu configs
  const memberMenu = [
    { id: "home", icon: Home, label: "Dashboard", path: "/member" },
    { id: "company", icon: Building2, label: "Company", path: "/member/company" },
    { id: "tasks", icon: UserCheck, label: "My Tasks", path: "/member/tasks" },
  ];

  const workspaceMenu = [
    { id: "overview", icon: Home, label: "Overview", path: `/member/workspace/${workspaceId}` },
    {
      id: "projects",
      icon: FolderKanban,
      label: "Projects",
      children: [
        { name: "Project Alpha", path: `/member/workspace/${workspaceId}/project-a` }, // Mock data
        { name: "Project Beta", path: `/member/workspace/${workspaceId}/project-b` },
      ],
    },
    { id: "members", icon: ClipboardCheck, label: "Members", path: `/member/workspace/${workspaceId}/members` },
    { id: "settings", icon: Settings, label: "Settings", path: `/member/workspace/${workspaceId}/settings` },
  ];
    ...
    <>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-6 custom-scrollbar">
          ...
          {/* SECTION: DEPARTMENTS LIST (Only in Core) */}
          {!isWorkspaceView && !collapsed && (
              <div className="pt-4 border-t border-slate-200 mx-1">
                  <div className="px-2 mb-2 text-[11px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                      ...
                      <span className="bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded text-[10px]">{workspaces.length}</span>
                  </div>
                  <div className="space-y-1">
                      {loading ? (
                          ...
                          <button
                              ...
                              className="w-full flex items-center gap-2 px-3 py-2 rounded-md text-slate-600 hover:bg-slate-200/60 hover:text-slate-900 transition-colors text-sm"
                          >
                              <div className="w-2 h-2 rounded-full bg-slate-400"></div>
                              <span className="truncate flex-1 text-left">{ws.workspaceName}</span>
                              {ws.roleCode?.includes("ADMIN") && <Crown className="w-3 h-3 text-amber-500" />}
                          </button>
                      ))}
                  </div>
              </div>
