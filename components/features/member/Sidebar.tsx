...
export default function MemberSidebar({
    ...
    const memberMenu = [
        ...
        { id: "tasks", icon: UserCheck, label: "Việc của tôi", path: "/member/tasks" },
    ];

    // 🔹 Menu khi đã vào 1 workspace
    const workspaceMenu = [
        { id: "overview", icon: Home, label: "Tổng quan", path: `/member/workspace/` },
        ...
        { id: "create", icon: PlusCircle, label: "Tạo dự án", path: `/member/workspace/${workspaceId}/create-project` },
        { id: "members", icon: ClipboardCheck, label: "Thành viên", path: `/member/workspace/${workspaceId}/members` },
        { id: "settings", icon: Settings, label: "Cài đặt", path: `/member/workspace/${workspaceId}/settings` },
    ];
                                    ...
                                    {loading ? (
                                        ...
                                        <div className="space-y-1 max-h-64 overflow-y-auto custom-scrollbar">
                                            {workspaces.map((ws, index) => {
                                                const isActive = pathname === `/member/workspace/${ws.workspaceId}`;
                                                const isAdmin = ws.roleCode?.includes("ADMIN");
                                                return (
                                                    <button
                                                        key={ws.workspaceId}
                                                        onClick={() => {
                                                            router.push(`/member/workspace/${ws.workspaceId}`);
                                                            if (window.innerWidth < 1024) onClose();
                                                        }}
