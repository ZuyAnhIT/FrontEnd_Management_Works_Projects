...
import { 
  getProjectMembers, 
  ProjectMember 
} from "@/services/apiProject"; // Lấy member cho filter
import { 
...
export default function WeeklyOverview({ projectId }: { projectId: number }) {
  ...
  const [filters, setFilters] = useState<OverviewParams>(() => {
      const end = new Date();
  ...
  useEffect(() => {
     if(projectId) {
         getProjectMembers(0, 0, projectId, { size: 100 }) // Lưu ý: apiProject cũ yêu cầu companyId/workspaceId, nếu đã sửa apiProject chỉ cần projectId thì tốt. Nếu chưa, hãy truyền đúng ID hoặc sửa apiProject.
         // Tạm thời giả định bạn đã sửa getProjectMembers chỉ cần projectId hoặc logic tương tự. 
         // Nếu chưa, bạn cần truyền companyId/workspaceId vào component này từ page cha.
         // Ở đây tôi dùng cách gọi an toàn nếu apiProject đã update:
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
        ...
        try {
            ...
            // Tự động chuyển tab nếu tab hiện tại không có dữ liệu nhưng tab khác có
            if (activeTab === "due" && res.dueSoonCount === 0 && res.updatedCount > 0) setActiveTab("updated");
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
  ...
  }, [projectId, filters]); // Re-fetch khi filters thay đổi

  // --- Logic Render ---
  if (!data && loading) return <div className="flex justify-center py-10"><Loader2 className="w-8 h-8 animate-spin text-blue-600"/></div>;
  
  // Xác định danh sách task cần hiển thị
