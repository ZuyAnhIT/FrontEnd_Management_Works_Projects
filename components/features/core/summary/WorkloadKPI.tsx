...
export default function WorkloadKPI({ data, unit }: WorkloadKPIProps) {
  ...
  // Sử dụng activeMembers cho topUser, nếu không có active member, dùng data[0] (sẽ có totalLoad=0)
  const topUser = (activeMembers.length > 0 ? activeMembers : data).reduce(
    (prev, current) => (prev.totalLoad > current.totalLoad ? prev : current),
    activeMembers.length > 0 ? activeMembers[0] : data[0]
  );
