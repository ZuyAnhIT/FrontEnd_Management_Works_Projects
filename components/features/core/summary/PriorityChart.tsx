...
// Thứ tự ưu tiên hiển thị (từ trên xuống dưới trong biểu đồ ngang)
const PRIORITY_ORDER = ["URGENT", "HIGH", "MEDIUM", "LOW"];

// =============================================================================
// 2. SUB-COMPONENT: Custom Tooltip
// =============================================================================
...
export default function PriorityChart({ data, loading }: PriorityChartProps) {
  ...
  // Sắp xếp dữ liệu để URGENT luôn nằm trên cùng (Recharts vẽ từ dưới lên)
  const sortedData = [...data].sort((a, b) => {
      const indexA = PRIORITY_ORDER.indexOf(a.code || "");
      const indexB = PRIORITY_ORDER.indexOf(b.code || "");
      return indexB - indexA; // Đảo ngược index để URGENT (index 0) nằm ở cuối mảng (vị trí trên cùng)
  });

  const totalTasks = data.reduce((acc, cur) => acc + cur.taskCount, 0);
