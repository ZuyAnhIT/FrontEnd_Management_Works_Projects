...
import { 
    X, Loader2, AlertTriangle, Trash2, Check 
} from "lucide-react";
...
export default function TagModal({ 
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.error(error);
            showToast("Cannot delete tag in use", "error");
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white rounded-xl shadow-2xl w-[400px] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-4 space-y-4">
                    ...
                    {!isDeleteMode ? (
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {TAG_COLORS.map(c => (
                                        <button 
                                            key={c} 
                                            className={`w-6 h-6 rounded-full transition-all flex items-center justify-center ${color === c ? 'ring-2 ring-offset-2 ring-slate-400 scale-110' : 'hover:scale-110'}`} 
                                            style={{backgroundColor: c}} 
                                            onClick={() => setColor(c)}
                                        >
                                            {color === c && <Check className="w-3 h-3 text-white stroke-[3px]" />}
                                        </button>
                                    ))}
                                </div>
                            </div>
                        ...
                        /* --- DELETE CONFIRMATION UI (Giống EpicModal) --- */
                        <div className="space-y-4 animate-in slide-in-from-right-4 duration-200">
                            <div className="bg-red-50 p-4 rounded-md flex gap-3 border border-red-100">
                                ...
                                <div className="text-sm text-red-800">
                                    ...
                                    <p className="mt-1 text-xs">Tag <strong>"{tag.name}"</strong> sẽ bị xóa vĩnh viễn khỏi tất cả các task.</p>
                                </div>
                            </div>
                            <div className="space-y-2">
