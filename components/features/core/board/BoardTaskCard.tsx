...
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"; // Dùng Avatar component cho đẹp
...
const AssigneeDropdown = ({ 
  ...
  return (
    // 🛑 QUAN TRỌNG: Ngăn chặn Drag (dnd-kit) và Click (Modal) tại đây
    <div 
      className="relative" 
      ref={dropdownRef}
      onPointerDown={(e) => e.stopPropagation()} // Chặn kéo thả
      onClick={(e) => e.stopPropagation()}      // Chặn mở modal task detail
    >
      <button
        type="button"
        onClick={(e) => {
            e.stopPropagation(); // Chắc chắn chặn click
            setIsOpen(!isOpen);
        }}
        className={`
        ...
        {currentUser ? (
          <Avatar className="w-6 h-6 border-2 border-white">
             {currentUser.avatarUrl || currentUser.avatar ? (
                <AvatarImage src={currentUser.avatarUrl || currentUser.avatar} className="object-cover"/>
             ) : null}
      ...
      {isOpen && (
        <div 
          // z-[9999] để đè lên các card khác trong cột
        ...
        >
          ...
          <div className="max-h-[200px] overflow-y-auto py-1 custom-scrollbar">
            ...
            {filteredUsers.length > 0 && <div className="h-px bg-slate-100 my-1 mx-2"></div>}
              ...
              filteredUsers.map((user) => {
                 ...
                 return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 text-xs transition-colors flex items-center gap-2 ${isSelected ? 'bg-blue-50 text-blue-700' : 'text-slate-700 hover:bg-slate-50'}`}
                  >
...
// --- MAIN BOARD TASK CARD ---
export default function BoardTaskCard({ task, index, users = [], onClick, onUpdateSuccess }: BoardTaskCardProps) {
  ...
  // ✅ FIX DỮ LIỆU: Lấy ID từ nested object thay vì flat field
  // task.assignee là object { id, name... }, task.assigneeId (không tồn tại hoặc undefined)
  ...
  // ✅ FIX RELOAD: Sync state khi props task thay đổi
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="p-3 pb-2">
        ...
        {/* Footer: Task Code, Priority, Assignee */}
        <div className="flex items-center justify-between min-h-[24px]">
          ...
          <div className="flex items-center gap-1.5 relative z-10">
            ...
            <div 
              ...
              onPointerDown={(e) => e.stopPropagation()} // Chặn kéo
            >
