...
// Helper: Xử lý đường dẫn ảnh (giữ nguyên logic gốc)
            ...
            <div className="group flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-lg hover:border-blue-400 hover:shadow-md transition-all duration-200">
                ...
                <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-2">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wide ${projectStatusStyle}`}>
                            {p.status || "UNKNOWN"}
                        </span>
                    </div>
        ...
        `}>
            ...
            >
                ...
                <div className="absolute top-3 right-3">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wide shadow-sm ${cardPriorityStyle}`}>
                        {p.priority}
                    </span>
                </div>
            </div>
            ...
            <div className="p-5 flex flex-col flex-1">
                              ...
                              <div className="absolute right-0 top-8 w-40 bg-white rounded-lg shadow-lg border border-slate-100 z-20 py-1 animate-in fade-in zoom-in-95">
                                    {!isTrash ? (
                                        <>
                                            ...
                                            <button 
                                                onClick={() => { setIsMenuOpen(false); onDelete(p.id); }}
                                        ...
                                        <button 
                                            onClick={() => { setIsMenuOpen(false); onRestore && onRestore(p.id); }}
                ...
                <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-xs text-slate-500 mt-auto pt-4 border-t border-slate-100">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="truncate max-w-[100px]" title={p.managerName || "Unassigned"}>{p.managerName || "Unassigned"}</span>
                    </div>
                    
                    <div className="flex items-center gap-2 justify-end">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase ${projectStatusStyle}`}>
                            {p.status}
                        </span>
                    </div>
