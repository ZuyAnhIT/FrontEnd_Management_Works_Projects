...
import { Search, MoreHorizontal, ChevronDown, Plus, ListFilter, Layers, LayoutList } from "lucide-react"
...
export default function TaskListPage() {
  ...
  // State for managing custom columns
  ...
  return (
    <div className="h-full flex flex-col bg-white">
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex items-center -space-x-1.5 ml-2">
            {[
              ...
              <div
                key={i}
                className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] text-white font-bold border-2 border-white ${user.bg} cursor-pointer hover:z-10 hover:scale-110 transition-transform`}
                title={user.name}
              >
                {user.name}
              </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="text-slate-600 hover:bg-slate-100 h-8 text-xs font-medium px-2"
              >
      ...
      <div className="flex-1 overflow-hidden flex relative">
        <div
          className="flex-1 overflow-y-auto overflow-x-auto bg-white w-full"
          style={{
            scrollbarWidth: "auto",
        ...
        >
          <table className="min-w-max w-full  text-left border-collapse">
            <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 z-20 text-[11px] font-bold text-slate-500 uppercase tracking-wider shadow-sm">
              <tr>
                <th className="px-4 py-2 w-12 text-center bg-slate-50">{/* Checkbox header */}</th>
                <th className="px-4 py-2 w-12 text-center whitespace-nowrap">Type</th>
                <th className="px-4 py-2 w-24 whitespace-nowrap">Key</th>
                <th className="px-4 py-2 min-w-[300px] whitespace-nowrap">Summary</th>
                <th className="px-4 py-2 w-32 whitespace-nowrap">Status</th>
                <th className="px-4 py-2 w-24 whitespace-nowrap">Comments</th>
                <th className="px-4 py-2 w-32 whitespace-nowrap">Sprint</th>
                <th className="px-4 py-2 w-28 whitespace-nowrap">Priority</th>
                <th className="px-4 py-2 w-28 whitespace-nowrap">Estimate</th>
                <th className="px-4 py-2 w-40 whitespace-nowrap">Progress</th>
                <th className="px-4 py-2 w-32 whitespace-nowrap">Assignee</th>
                <th className="px-4 py-2 w-32 whitespace-nowrap">Due Date</th>
                <th className="px-4 py-2 w-40 whitespace-nowrap">Labels</th>
                <th className="px-4 py-2 w-32 whitespace-nowrap">Created</th>
                
                {customColumns.map((col, idx) => (
                  <th key={`custom-${idx}`} className="px-4 py-2 w-32 whitespace-nowrap">
                    {col}
                  </th>
                ))}
                <th className="px-4 py-2 w-12 text-center sticky right-0 bg-slate-50">
                  <button
                    onClick={() => setIsColumnDialogOpen(true)}
                    className="inline-flex items-center justify-center w-6 h-6 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded transition-colors"
                    title="Add Column"
                  >
            ...
            <tbody className="divide-y divide-slate-100">
              {Object.entries(groupedTasks).flatMap(([groupKey, tasks]) => {
                ...
                if (groupBy !== "none") {
                  groupRows.push(
                    <tr key={`group-${groupKey}`} className="bg-slate-50/50 border-b border-slate-200">
                      <td colSpan={18 + customColumns.length} className="px-4 py-2">
                ...
                groupRows.push(
                  ...tasks.map((task) => (
                    <TaskRow key={task.id} task={task} onTaskSelect={() => handleTaskSelect(task)} />
                  )),
                )
              ...
              <tr className="hover:bg-slate-50 cursor-pointer group border-b border-transparent">
                <td colSpan={18 + customColumns.length + 1} className="px-4 py-2">
        ...
        {isModalOpen && selectedTask && (
          <div className="absolute inset-0 z-50 pointer-events-none flex justify-end">
            <div className="pointer-events-auto h-full">
              <TaskDetailWrapper task={selectedTask} isOpen={isModalOpen} onClose={handleCloseModal} />
            </div>
          </div>
        )}
      </div>
      ...
      {isColumnDialogOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg p-6 w-96">
            <h2 className="text-lg font-bold text-slate-900 mb-4">Add New Column</h2>
            <input
              type="text"
              id="columnName"
              placeholder="Enter column name..."
              className="w-full px-4 py-2 border border-slate-300 rounded-lg mb-4 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
              onKeyDown={(e) => {
                if (e.key === "Enter" && e.currentTarget.value.trim()) {
                  handleAddColumn(e.currentTarget.value.trim())
