...
import { X, Loader2, Calendar, Target, Layers, CheckCircle2 } from "lucide-react";
import { getSprintDetails, updateSprint, SprintDetails, UpdateSprintPayload } from "@/services/apiSprint";
...
export default function SprintDetailModal({ 
  projectId, sprintId, onClose, onUpdate 
       ...
       >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
             <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Sprint Details</span>
                {isSaving && <span className="text-xs text-blue-600 flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin"/> Saving...</span>}
             </div>
             <button onClick={onClose} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded transition-colors">
                <X className="w-5 h-5"/>
             </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
             {loading ? (
                <div className="h-40 flex items-center justify-center"><Loader2 className="w-8 h-8 text-blue-600 animate-spin"/></div>
             ) : sprint ? (
                <div className="space-y-6">
                   ...
                   <div>
                      ...
                      <div className="mt-2">
                         <span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wide ${
                            sprint.status === 'IN_PROGRESS' ? 'bg-green-100 text-green-700 border-green-200' :
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg flex items-center gap-3">
                         <div className="p-2 bg-white rounded shadow-sm text-blue-600"><Layers className="w-5 h-5"/></div>
                         <div>
                            <p className="text-[10px] text-slate-500 uppercase font-bold">Story Points</p>
                            <p className="text-lg font-bold text-slate-800 leading-none mt-0.5">{sprint.totalStoryPoints || 0}</p>
                         </div>
                      </div>
                      <div className="p-3 bg-slate-50 border border-slate-100 rounded-lg flex items-center gap-3">
                         <div className="p-2 bg-white rounded shadow-sm text-green-600"><CheckCircle2 className="w-5 h-5"/></div>
                         <div>
                            <p className="text-[10px] text-slate-500 uppercase font-bold">Total Issues</p>
                            <p className="text-lg font-bold text-slate-800 leading-none mt-0.5">{sprint.taskCount || 0}</p>
                         </div>
                      </div>
                   </div>
                ...
                <div className="h-40 flex items-center justify-center text-slate-500">Sprint not found</div>
             )}
          </div>
