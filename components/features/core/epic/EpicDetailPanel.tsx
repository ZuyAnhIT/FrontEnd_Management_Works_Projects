...
// Helper: Convert ISO Date to YYYY-MM-DD for input fields
...
const getStatusStyle = (status?: string) => {
    switch(status) {
        ...
        default: return 'bg-slate-100 text-slate-600 border-slate-200'; // OPEN
    }
};
...
// Omit 'color' if not used in this form
...
export default function EpicDetailPanel({ projectId, epicId, onClose, onUpdate }: EpicDetailPanelProps) {
  ...
  useEffect(() => {
    if (epicId) {
      ...
      apiEpic.getEpicDetail(projectId, epicId)
        .then(data => {
            setEpic(data);
            // Sync form data with fetched epic
            setFormData({
                name: data.name,
  ...
  // --- HANDLER: UPDATE ---
  const handleUpdate = async (field: keyof EpicFormData, value: any) => {
    if (!epic) return;

    // 1. Optimistic Update (Update local state immediately)
    setFormData(prev => ({ ...prev, [field]: value }));

    // 2. Prepare Payload (Handle Date fields)
    ...
    try {
       ...
       // Update 'epic' state to match server (optional but good practice)
       ...
       // Revert optimistic update here if needed (omitted for simplicity)
    } finally {
       setIsSaving(false);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
             {loading ? (
                ...
                <div className="p-5 pb-20 space-y-6">
                    ...
                    {/* 1. TITLE INPUT (Auto-resize Textarea) */}
                    <textarea 
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-[100px_1fr] items-center gap-2">
                            ...
                            <div className="relative w-fit">
                                <select 
                                    className={`appearance-none pl-3 pr-8 py-1.5 rounded-md text-xs font-bold border cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-100 transition-all ${getStatusStyle(formData.status)}`}
