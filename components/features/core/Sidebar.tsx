        ...
        <>
            ...
            >
                ...
                <div className="h-16 flex items-center justify-between px-4 border-b border-slate-200/50 bg-[#F4F5F7] shrink-0">
                    ...
                    <button onClick={onClose} className="lg:hidden p-1.5 rounded-md hover:bg-slate-200 text-slate-500 transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-6 custom-scrollbar">
                                ...
                                <div key={item.id}>
                                    <div className="flex items-center">
                                        <button
                                            onClick={() => {
                                                if (item.path) {
                                                    setActiveMenu(item.path); // Update active state immediately
                                                    router.push(item.path);
                                                    if (window.innerWidth < 1024) onClose();
                                                }
                                            }}
                                            className={`
                                                group relative flex-1 flex items-center rounded-md transition-all duration-200 
                                                ${collapsed ? 'justify-center px-0 py-3' : 'px-3 py-2 gap-3'} 
                                                ${isActive ? 'bg-blue-100 text-blue-700' : 'text-slate-600 hover:bg-slate-200/60 hover:text-slate-900'}
                                            `}
                                            title={collapsed ? item.label : undefined}
                                        >
                                            {isActive && <div className="absolute left-0 top-1 bottom-1 w-1 bg-blue-600 rounded-r-full"></div>}
                                            <item.icon className={`${collapsed ? 'w-5 h-5' : 'w-4 h-4'} ${isActive ? 'text-blue-700' : 'text-slate-500 group-hover:text-slate-700'} transition-colors shrink-0`} />
                    ...
                    {/* SECTION 2: WORKSPACES LIST (Only visible on Member View and not collapsed) */}
                                ...
                                {workspaces.length > 0 ? (
                                    workspaces.map((ws) => (
                                        <button
                                            key={ws.id}
                                            onClick={() => {
                                                router.push(`/core/workspace/${ws.id}`);
                                                if (window.innerWidth < 1024) onClose();
                                            }}
                                        ...
                                        >
                                            <div className="w-2 h-2 rounded-full bg-slate-400 group-hover:bg-blue-500 transition-colors shrink-0"></div>
                ...
                <div className="p-4 border-t border-slate-200 bg-[#F4F5F7] shrink-0">
                    ...
                    >
                        {collapsed ? (
                            ...
                            <>
                                <div className="flex items-center justify-center w-6 h-6 rounded bg-slate-200 text-slate-500 group-hover:text-slate-700 shrink-0">
            ...
            {/* Placeholder div (Đẩy nội dung chính sang phải) */}
