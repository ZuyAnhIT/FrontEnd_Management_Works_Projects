...
export default function CoreHeader({ onMenuToggle, user }: HeaderProps) {
  ...
  return (
    // Header Container: Nền trắng, border mỏng
        ...
        <div className="flex items-center gap-4">
          {isWorkspaceDetail ? (
            ...
            <>
              ...
              {/* Logo & Brand */}
              <div className="flex items-center gap-2 cursor-pointer group" onClick={() => router.push("/core")}>
                <div className="w-8 h-8 bg-blue-600 rounded-md flex items-center justify-center shadow-sm">
                  <LayoutDashboard className="w-5 h-5 text-white" />
                </div>
                <div className="hidden sm:block">
                   <span className="font-bold text-lg text-slate-800 tracking-tight block leading-none">WorkNet</span>
                   <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Core Platform</span>
                </div>
              </div>
            </>
          )}
