  ...
  return (
    <div className="relative group w-full">
      <select
        ...
        } focus:ring-2 focus:ring-offset-1 outline-none cursor-pointer uppercase transition-all`}
        value={value}
...
export default function TaskDetailPanel({
  ...
  // Click outside Tag Popover (Logic nghiệp vụ quan trọng)
  useEffect(() => {
  ...
  // LOAD DATA (Logic nghiệp vụ quan trọng)
  useEffect(() => {
    if (taskId) {
      setLoading(true);
      ...
      getTaskDetails(taskId)
        .then((data) => {
          // 1. 🔥 TÍNH TOÁN CÁC ID AN TOÀN TRƯỚC (Logic quan trọng)
  ...
  // --- TAG HANDLERS (Logic nghiệp vụ quan trọng) ---
  ...
  // --- MAIN UPDATE HANDLER (Logic nghiệp vụ quan trọng) ---
  const handleUpdate = async (field: keyof UpdateTaskData, value: any) => {
    ...
    // 2. OPTIMISTIC UI UPDATES (Fix Assignee, Status)
    if (field === "assigneeId") {
      ...
      setTask((prev) =>
        prev
          ? {
              ...
              assigneeName:
                userId === 0 ? "Unassigned" : user?.fullName || user?.name,
              assigneeAvatar:
                userId === 0 ? null : user?.avatarUrl || user?.avatar,
            }
          : null
      );
    }
  ...
  // Subtask Handlers (Logic nghiệp vụ quan trọng)
  ...
  const handleEpicChange = async (selectedEpic: any | null) => {
    ...
    try {
      ...
      setTask(updatedTaskData as ExtendedTaskDetail); // Cập nhật lại state bằng dữ liệu chuẩn từ Backend
      if (onUpdate) onUpdate();
      ...
      showToast(message, "error");
      fetchSubtasks(taskId!); // Revert nếu cần
    } finally {
      setIsSaving(false);
    }
  };

  // --- HÀM NÀY ĐỂ XỬ LÝ UPDATE SUBTASK (ASSIGNEE, TITLE...) ---
  const handleUpdateSubtask = async (subTaskId: number, data: any) => {
    if (!taskId) return;

    // 1. Optimistic Update (Logic nghiệp vụ quan trọng)
    setSubtasks((prev) =>
      prev.map((s) => {
        if (s.id === subTaskId) {
          if (data.assigneeId !== undefined) {
            ...
            const member = members.find(
              (m) => (m.userId || m.id) === Number(data.assigneeId)
            );
            return {
              ...s,
              ...data,
                ...
                {task && (
                  <div
                    className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer transition-colors group ${
                      task.epic ? "hover:bg-purple-50" : "hover:bg-slate-100"
                    }`}
                    ...
                    {task.epic ? (
                      <>
                        <div
                          ...
                          style={{
                            backgroundColor: task.epic.color || "#a855f7",
                          }}
                        ></div>
                        <span
