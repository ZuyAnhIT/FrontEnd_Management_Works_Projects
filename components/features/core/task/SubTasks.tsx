...
const getAvatarColor = (id: number | string | null) => {
    ...
    const hash = typeof id === "number" ? id : id ? id.toString().split("").reduce((acc, char) => acc + char.charCodeAt(0), 0) : 0;
...
const PriorityIcon = ({ priority }: { priority?: string }) => {
    ...
    return <div title={p} className="cursor-help">{icon}</div>;
};

// =============================================================================
// 3. SUB-COMPONENT: MiniAssigneeDropdown
// =============================================================================
...
const MiniAssigneeDropdown = ({
    ...
    return (
        <div className="relative flex justify-center" ref={dropdownRef} onClick={(e) => e.stopPropagation()}>
            <TooltipProvider>
                <Tooltip delayDuration={300}>
                    <TooltipTrigger asChild>
                        <div
                            className={`transition-transform ${readOnly ? 'cursor-default opacity-80' : 'cursor-pointer hover:scale-110'}`}
                            onClick={() => !readOnly && setIsOpen(!isOpen)} // ✅ Disable click if ReadOnly
                        >
            ...
            {/* Dropdown List - Only render if not ReadOnly */}
            {isOpen && !readOnly && (
                <div className={`absolute right-0 w-60 bg-white rounded-lg shadow-xl z-50 border border-slate-200 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-100 ${isLastRow ? "bottom-full mb-2" : "top-full mt-1"}`}>
                    ...
                    <div className="max-h-56 overflow-y-auto py-1">
                        {"unassigned".includes(keyword.toLowerCase()) && (
                            <div className="px-3 py-2 hover:bg-slate-50 cursor-pointer flex items-center gap-2 text-xs text-slate-600 border-b border-slate-100" onClick={(e) => handleSelect(e, null)}>
                        ...
                        {filteredMembers.length > 0 ? (
                            filteredMembers.map((user) => {
                                ...
                                return (
                                    <div key={userId} className={`px-3 py-2 cursor-pointer flex items-center gap-2 text-xs transition-colors ${isSelected ? "bg-blue-50 text-blue-700" : "hover:bg-slate-50 text-slate-700"}`} onClick={(e) => handleSelect(e, userId)}>
                                        <Avatar className="w-6 h-6"><AvatarImage src={user.avatarUrl || user.avatar} className="object-cover" /><AvatarFallback className={`text-white text-[8px] ${getAvatarColor(userId)}`}>{user.fullName ? user.fullName.substring(0, 2).toUpperCase() : "U"}</AvatarFallback></Avatar>
                                        <div className="flex flex-col min-w-0"><span className="truncate font-medium">{user.fullName || user.name}</span></div>
...
export default function TaskSubtasks({
    ...
    // Helpers: Get Status Color
    const getStatusColor = (status: string) => {
        const s = status ? status.toUpperCase() : "TODO";
        if (s === "DONE") return "bg-green-100 text-green-700 hover:bg-green-200 border border-green-200";
        if (s === "IN_PROGRESS" || s === "IN PROGRESS") return "bg-blue-100 text-blue-700 hover:bg-blue-200 border border-blue-200";
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <div className="h-2 w-32 bg-slate-200 rounded-full overflow-hidden relative">
                        <div className="h-full bg-green-600 transition-all duration-500 ease-out" style={{ width: `${progressPercent}%` }}></div>
                    </div>
                ...
                <table className="w-full text-sm text-left border-collapse table-fixed">
                    <thead className="bg-slate-50/80 text-[11px] font-semibold text-slate-500 uppercase tracking-wide border-b border-slate-200">
                        <tr>
                            ...
                            {/* Hide Delete Column Header if ReadOnly */}
                            {!isReadOnly && <th className="py-2.5 px-1 font-semibold w-[30px]"></th>}
                        </tr>
                    </thead>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {subtasks.length > 0 ? (
                                    ...
                                    {editingSubtaskId === sub.id ? (
                                        <td colSpan={isReadOnly ? 4 : 5} className="p-1 pl-2">
                                            <div className="flex items-center gap-2 w-full bg-white z-10 relative p-1">
                                                ...
                                                <div className="flex shrink-0">
                                                    <button onClick={saveEditing} className="p-1.5 hover:bg-green-100 text-green-600 rounded" title="Save changes"><Check className="w-4 h-4" /></button>
                                                    <button onClick={cancelEditing} className="p-1.5 hover:bg-red-100 text-red-600 rounded" title="Cancel editing"><X className="w-4 h-4" /></button>
                                                </div>
                                            </div>
                                        </td>
                                    ) : (
                                        <>
                                            ...
                                            <td className="py-2 pl-4 pr-2 align-middle">
                                                <div className={`flex items-center gap-3 min-w-0 w-full ${isReadOnly ? '' : 'cursor-pointer group/title'}`} onClick={() => !isReadOnly && onToggleStatus(sub)}>
                                                    <CheckSquare className={`w-4 h-4 shrink-0 transition-colors ${sub.status === "DONE" ? "text-green-600" : "text-blue-500"} ${isReadOnly ? 'opacity-70' : ''}`} />
                                                    <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
                                                        ...
                                                        <TooltipProvider>
                                                            <Tooltip delayDuration={300}>
                                                                <TooltipTrigger asChild>
                                                                    <span
                                                                        onClick={(e) => { e.stopPropagation(); if (onViewDetail) onViewDetail(sub.id); }}
                                                                        className={`truncate text-slate-700 font-medium text-sm block w-full cursor-pointer hover:text-blue-600 hover:underline ${sub.status === "DONE" ? "line-through text-slate-400" : ""}`}
                                                                    >
                                                                        {truncateString(sub.title, 65)}
                                                                    </span>
                                                                </TooltipTrigger>
                                                                <TooltipContent className="bg-slate-800 text-white text-xs px-2 py-1 max-w-[300px] break-all">{sub.title}</TooltipContent>
                                                            </Tooltip>
                                                        </TooltipProvider>
                                                    </div>
                                                    ...
                                                    {!isReadOnly && (
                                                        <button onClick={(e) => { e.stopPropagation(); startEditing(sub); }} className="opacity-0 group-hover/title:opacity-100 p-1.5 hover:bg-slate-200 rounded text-slate-500 transition-all shrink-0" title="Edit title"><Edit2 className="w-3.5 h-3.5" /></button>
                                                    )}
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-2 px-1 text-center align-middle border-l border-transparent group-hover:border-slate-100">
                                                <div className="flex justify-center">
                                                    <div
                                                        className={`inline-flex items-center justify-center gap-1 px-2 py-1 rounded text-[10px] font-bold transition-all select-none w-full max-w-[100px] shadow-sm border ${getStatusColor(sub.status)} ${isReadOnly ? 'cursor-default opacity-80' : 'cursor-pointer'}`}
                                                        onClick={() => !isReadOnly && onToggleStatus(sub)} // ✅ Disable click
                                                        title={isReadOnly ? undefined : "Click to toggle status"}
                                                    >
                                                        <span className="whitespace-nowrap truncate">{sub.status?.replace(/_/g, " ") || "TO DO"}</span>
                                                        {!isReadOnly && <ChevronDown className="w-3 h-3 opacity-50 shrink-0 hidden sm:block" />}
                                                    </div>
                                                </div>
                                            </td>

                                            {/* 5. Delete - Hide if ReadOnly */}
                                            {!isReadOnly && (
                            ...
                            <tr>
                                <td colSpan={isReadOnly ? 4 : 5} className="py-10 text-center text-xs text-slate-400 italic bg-slate-50/30">
                                    No subtasks created yet.
                                    {/* ✅ Hide quick create link if ReadOnly */}
                                    {!isReadOnly && (
