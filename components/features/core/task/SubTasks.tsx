...
const getAvatarColor = (id: number | string | null) => {
  ...
  // Sử dụng hash đơn giản cho màu sắc nhất quán
  const hash =
    typeof id === "number"
      ? id
      : id
...
// =============================================================================
// 3. SUB-COMPONENT: MiniAssigneeDropdown (Logic phức tạp)
// =============================================================================
...
const MiniAssigneeDropdown = ({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className={`absolute right-0 w-60 bg-white rounded-lg shadow-xl z-50 border border-slate-200 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-100 
                ...
                return (
                  ...
                  >
                    <Avatar className="w-6 h-6">
                      ...
                      />
                      <AvatarFallback
                        className={`text-white text-[8px] ${getAvatarColor(
                          userId
                        )}`}
                      >
                        {user.fullName
    ...
    <div className="space-y-3 pt-2">
      ...
      <div className="border border-slate-200 rounded-lg bg-white shadow-sm overflow-visible">
        <table className="w-full text-sm text-left border-collapse table-fixed">
          <thead className="bg-slate-50/80 text-[11px] font-semibold text-slate-500 uppercase tracking-wide border-b border-slate-200">
            <tr>
              ...
              {/* Các cột khác FIX CỨNG width */}
              <th className="py-2.5 px-1 font-semibold text-center w-[40px]">
                Pri
              </th>
              <th className="py-2.5 px-1 font-semibold text-center w-[80px]">
                Assignee
              </th>
              <th className="py-2.5 px-1 font-semibold text-center w-[110px]">
                Status
              </th>
          ...
          <tbody className="divide-y divide-slate-100">
            {subtasks.length > 0 ? (
              subtasks.map((sub, index) => (
                <tr
                  key={sub.id}
                  className="group hover:bg-slate-50 transition-colors h-10 relative"
                >
                  {editingSubtaskId === sub.id ? (
                    ...
                    <>
                      ...
                      <td className="py-2 pl-4 pr-2 align-middle">
                        <div
                          className="flex items-center gap-3 cursor-pointer group/title min-w-0 w-full"
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
                            <span className="text-slate-400 text-[10px] font-mono shrink-0 pt-0.5">
                              SUB-{sub.id}
                            </span>
                              ...
                              <Tooltip delayDuration={300}>
                                <TooltipTrigger asChild>
                                  <span
                                    ...
                                    className={`truncate text-slate-700 font-medium text-sm block w-full hover:text-blue-600 hover:underline cursor-pointer ${
                                      sub.status === "DONE"
                      ...
                      <td className="py-2 px-1 text-center align-middle border-l border-transparent group-hover:border-slate-100 overflow-visible">
                        <div className="flex justify-center relative z-10">
                          <MiniAssigneeDropdown
                            ...
                            members={members}
                            onUpdate={(newId: any) =>
                              onAssigneeChange(Number(sub.id), newId)
                            }
                            // BỔ SUNG: Truyền logic dòng cuối để Dropdown tự đảo chiều
                            isLastRow={
                              index >= subtasks.length - 2 &&
                              subtasks.length > 2
                            }
                          />
                        </div>
                      ...
                      <td className="py-2 px-1 text-center align-middle border-l border-transparent group-hover:border-slate-100">
                        <div className="flex justify-center">
                          <div
                            className={`inline-flex items-center justify-center gap-1 px-2 py-1 rounded text-[10px] font-bold cursor-pointer transition-all select-none w-full max-w-[100px] shadow-sm border ${getStatusColor(
                              sub.status
                            )}`}
                            onClick={() => onToggleStatus(sub)}
              ...
              <tr>
                <td
                  colSpan={5}
                  className="py-10 text-center text-xs text-slate-400 italic bg-slate-50/30"
                >
                  No subtasks created yet.
                  <button
                    onClick={onAddSubtask}
                    className="text-blue-600 hover:underline cursor-pointer ml-1 font-medium inline-flex items-center gap-1"
                  >
        ...
        {subtasks.length > 0 && (
          <div
            onClick={onAddSubtask}
            className="px-4 py-2.5 bg-slate-50/50 border-t border-slate-200 text-xs font-medium text-slate-500 hover:text-blue-600 hover:bg-slate-100 cursor-pointer flex items-center gap-2 transition-colors rounded-b-lg"
          >
