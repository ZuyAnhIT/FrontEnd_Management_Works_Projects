...
export default function TaskDetailModalFloating({
  ...
  // Refs for click outside (giữ nguyên logic)
  ...
  // Floating Window State (giữ nguyên logic drag/resize)
  ...
  // Click Outside Handler (giữ nguyên logic)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen && taskId) {
      ...
      // 4. FETCH SPRINTS (THÊM MỚI - Giữ nguyên)
  ...
  // Main Task Update (Logic nghiệp vụ quan trọng)
  const handleUpdate = async (field: keyof UpdateTaskData, value: any) => {
    if (!task || !taskId) return;
    setFormData((prev) => ({ ...prev, [field]: value }));

    // Optimistic UI: Assignee (Logic nghiệp vụ quan trọng)
    if (field === "assigneeId") {
    ...
    // Optimistic UI: Status (Logic nghiệp vụ quan trọng)
    if (field === "statusId") {
    ...
    // Reset ID fields to null if value is 0 (Logic nghiệp vụ quan trọng)
    if (
      ["sprintId", "epicId", "assigneeId"].includes(field) &&
    ...
    setTask((prev) =>
      prev
        ? {
            ...prev,
            epic: epic
              ? { id: epic.id, name: epic.name, color: epic.color }
              : null,
          }
        : null
    );
  ...
  const handleAddTag = async (tag: Tag) => {
    ...
    try {
      ...
      const message =
        error.message || error.response?.data?.message || "Failed to add tag";
      showToast(message, "error");
    }
  };
  ...
  const handleRemoveTag = async (tagId: number) => {
    ...
    setTask({ ...task, tags: newTags as any });
    try {
      await apiTag.removeTagFromTask(
        companyId,
        workspaceId,
        projectId,
        taskId,
  ...
  const handleToggleSubtask = async (subtask: Subtask) => {
    ...
    // Logic nghiệp vụ quan trọng: TO_DO -> IN_PROGRESS -> DONE -> TO_DO
    ...
    const newStatus = statusCycle[currentStatus] || "TO_DO";
  ...
  const handleUpdateSubtask = async (subTaskId: number, data: any) => {
    ...
    // Optimistic Update (Logic nghiệp vụ quan trọng)
    setSubtasks((prev) =>
      prev.map((s) => {
        if (s.id === subTaskId) {
    ...
    <>
      ...
      >
        ...
        {selectedSubtaskId && taskId ? (
          ...
          /* NẾU KHÔNG: Hiện màn hình Task cha */
          <>
            {/* --- HEADER --- */}
            <div
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar border-r border-slate-100">
                    <div className="p-8 space-y-8">
                          ...
                          <div className="border border-blue-500 rounded-md p-2">
                            ...
                            <div className="flex justify-end gap-2 mt-2">
                              <Button
                                ...
                                disabled={
                                  isSaving || !formData.description?.trim()
                                }
                              >
                                Save
                              </Button>
                      ...
                      <TaskSubtasks
                        ...
                        onAssigneeChange={(subTaskId, userId) =>
                          handleUpdateSubtask(subTaskId, { assigneeId: userId })
                        }
                        onEditContent={(subTaskId, data) =>
                          handleUpdateSubtask(subTaskId, data)
                        }
                        onViewDetail={(subId) => setSelectedSubtaskId(subId)}
                        // isSaving={isSaving} // Giữ nguyên prop isSaving nếu component con hỗ trợ
                      />
                  ...
                  <div className="w-[340px] flex flex-col overflow-y-auto custom-scrollbar bg-slate-50/50 shrink-0">
                    <div className="p-6 space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <select
                            ...
                            onChange={(e) =>
                              handleUpdate("statusId", Number(e.target.value))
                            }
                            style={{
                              color: task.status?.color || "inherit",
                              borderLeftWidth: "4px",
                              borderLeftColor:
                                task.status?.color || "transparent",
                            }}
                            disabled={isSaving}
                          >
                            {statuses.length > 0 ? (
                      ...
                      <div className="space-y-3 relative">
                        ...
                        {isTagPopoverOpen && (
                          <div
                            className="absolute right-0 top-8 z-20 w-56 bg-white rounded-md shadow-xl border border-slate-200 p-2 animate-in fade-in zoom-in-95 duration-100"
                            ref={tagPopoverRef}
                          >
                            <Input
                              ref={tagInputRef}
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter" && tagSearch.trim()) {
                                  const exactMatch = allProjectTags.find(
                                    (t) =>
                                      t.name.toLowerCase() ===
                            ...
                            <div className="text-[10px] text-slate-400 px-2 py-1 border-b border-slate-50 uppercase font-bold">
                              Select an option
                            </div>
                            ...
                            <div className="max-h-40 overflow-y-auto custom-scrollbar p-1 space-y-0.5">
                              {filteredTags.length > 0 ? (
                                filteredTags
                                  .filter(
                                    (t) =>
                                      !(task.tags as unknown as Tag[])?.find(
                                        (tt) => tt.id === t.id
                                      )
                                  )
                                  .map((tag) => (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            style={
                              task?.epic
                                ? {
                                    borderLeftColor: task.epic.color || "#ccc",
                                    borderLeftWidth: "3px",
                                  }
                                : {}
                            }
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="relative w-[140px]">
                            <select
                              ...
                              onChange={(e) =>
                                handleUpdate("sprintId", Number(e.target.value))
                              }
                              disabled={isSaving}
                            >
                      ...
                      <div className="mt-auto pt-6 text-[10px] text-slate-400">
                        ...
                        {/* Giả định luôn là 'just now' sau mỗi update để đơn giản hóa UI */}
      ...
      <TagModal
        ...
        onDelete={(deletedTagId) => {
          setAllProjectTags((prev) =>
            prev.filter((t) => t.id !== deletedTagId)
          );
          setTask((prev) =>
            prev
              ? {
